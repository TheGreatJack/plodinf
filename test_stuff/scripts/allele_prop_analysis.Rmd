...
data <- read.table("/home/andresflorian/University/Cebolla/internship/ploidy/allelic_proportions_libraries/run01/AN023_P1_allele_prop_m15_d25.tsv", header = TRUE, skip = 2)
...
load_and_plot_wrapper <- function(data_path) {
  ...
  for (item_name in names(all_items)) {
    ...
    ploty <- ggplot(aes(x = as.numeric(values)), data = data.frame(values)) +
      geom_histogram(binwidth = 0.05, color="#e9ecef", alpha=0.7, position = 'identity') +
...
```{r}
data_path <- "/home/andresflorian/University/Cebolla/internship/ploidy/allelic_proportions_libraries/run01/"
