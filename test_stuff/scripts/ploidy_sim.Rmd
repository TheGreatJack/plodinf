...
compute_pmf_probs <- function(base_prob_table, read_depth_distr, peak_ratio){
  read_depth_table <- as.data.frame(table(read_depth_distr))
  
  for (read_depth in read_depth_table$read_depth_distr){
    # fitler base for a given read depth and get the average for each frequency, then ponder each average for the adjusted to peak ratio read depth fuck me, then return a table and then create another function that would bother with returning probabilities given a value, fuck. Then test the fitting with MASs 
  }
}
```
...
# Assuming your dataframe is called 'data'
data_long <- data %>%  # Pipe data frame
