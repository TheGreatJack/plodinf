...
optim_func_4n <- function(central_prop, base_probability_table = base_probability_test, read_depth_distrib = read_depth_distr_test, sim_table = simulation_table){
  ...
  final_pmf_weights_test <- compute_pmf_probs_4n(base_probability_table, read_depth_distrib, central_prop)
  
  colnames(sim_table) <- c("sim_freqs", "Freq", "probs")
  model_simu_merge <- merge_with_tol(final_pmf_weights_test, sim_table, tolerance = 1e-12)
...
read_depth_distr_test <- read_depth_distr
simulation_table <- tetraplod_test_table
optim_func_4n(central_prop = 0.8)
```
```{r}
...
model_plodies_evaluator <- function(sim_table){
  for (ploidy in c(2,3,4)){
